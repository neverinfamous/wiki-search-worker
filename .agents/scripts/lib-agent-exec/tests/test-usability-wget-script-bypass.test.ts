...
const payloadPath = path.join(os.tmpdir(), `test-wget-script-bypass-${Date.now()}.json`);
const agentExecPath = path.resolve(__dirname, "../agent-exec.ts");

describe("Usability Test: Agent-Exec Hallucinations (Network aliases bypass bug) Wget Script", () => {
  test("Intercepts 'wget' command within a 'script' payload type", () => {
    const scriptPath = path.join(os.tmpdir(), `test-wget-script-${Date.now()}.ps1`);
    fs.writeFileSync(scriptPath, "wget https://example.com");
