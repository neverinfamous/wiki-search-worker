...
const payloadPath = path.join(os.tmpdir(), `test-wget-bypass-${Date.now()}.json`);
const agentExecPath = path.resolve(__dirname, "../agent-exec.ts");

describe("Usability Test: Agent-Exec Hallucinations (Network aliases bypass bug) Wget", () => {
  test("Intercepts 'wget' command even when arguments are part of the command string", () => {
    const payload = {
      type: "command",
