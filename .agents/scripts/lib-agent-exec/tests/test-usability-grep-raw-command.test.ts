...
const payloadPath = path.join(os.tmpdir(), `test-grep-raw-hallucination-${Date.now()}.json`);
const agentExecPath = path.resolve(__dirname, "../agent-exec.ts");

describe("Usability Test: Agent-Exec Hallucinations (FS aliases)", () => {
  test("Intercepts 'grep pattern file.txt' command natively and throws anti-hallucination error", () => {
    const payload = {
      type: "command",
