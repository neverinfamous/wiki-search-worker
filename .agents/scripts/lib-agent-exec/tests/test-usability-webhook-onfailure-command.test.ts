...
test('Webhook Validation with array of onFailure URLs - command payload', async () => {
  ...
  const payloadPath = path.join(os.tmpdir(), 'test-webhook-onfailure-array-command.json');
  const payload = {
    type: 'command',
    command: 'git',
    args: ['invalid-cmd-to-fail'],
    onFailure: [
